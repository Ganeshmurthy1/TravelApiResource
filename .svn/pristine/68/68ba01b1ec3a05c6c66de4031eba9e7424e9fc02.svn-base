...
public class BusCreditNoteDaoImp implements BusCreditNoteDao ,Serializable{
	public static final Logger logger = Logger.getLogger(BusCreditNoteDaoImp.class);	
	/**
	 * By Yogesh
	 */
	...
	@Override
	public List<BusCreditNote> getCreditNoteByUser(int id)
			throws HibernateException {	
		List<BusCreditNote> userlist=null;
		try{
	...
	public BusCreditNote getCreditNoteById(String id) throws HibernateException {
		...
		try{
			...
			creditNote = (BusCreditNote) criteria.uniqueResult();	
		}catch (HibernateException e) {			
			throw e; 
		}finally {
			if(session != null && session.isOpen())
	...
	public BusCreditNote getCreditNoteByRowId(String id)
			throws HibernateException {
		Session sess = null;		
		BusCreditNote busCreditNote = null;				
		try{
			sess = sessionFactory.openSession();			
			Criteria criteria=sess.createCriteria(BusCreditNote.class);
			criteria.add(Restrictions.eq("id", Integer.parseInt(id)));
			busCreditNote = (BusCreditNote) criteria.uniqueResult();	
	...
	public BusCreditNote getCreditNoteDetailsByComapnyId(int companyid, Long id) {
		...
		try
		{
			...
			creditNoteObj = (BusCreditNote) criteria.uniqueResult();			
		}
		catch(Exception e){
			logger.error("---------Exception---------"+e.getMessage());
