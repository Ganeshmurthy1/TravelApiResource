...
public class FlightCreditNoteDaoImp implements FlightCreditNoteDao ,Serializable{
	public static final Logger logger = Logger.getLogger(FlightCreditNoteDaoImp.class);	
	/**
	 * By khalid
	 */
	...
	@Override
	public List<User> getTicketCancelByUser(FlightOrderRow userid)
		...
		try{
			...
			criteria.add(Restrictions.eq("id",Integer.parseInt(userid.getUserId())));			
	...
	@Override
	public List<FlightCreditNote> getCreditNoteByUser(int id)
			throws HibernateException {	
		List<FlightCreditNote> userlist = null;
		try{
			session = sessionFactory.openSession();			
			Criteria criteria=session.createCriteria(FlightCreditNote.class);
	...
	public FlightCreditNote getCreditNoteById(String id) throws HibernateException {
		FlightCreditNote flightCreditNote = null;			
		try{
			session = sessionFactory.openSession();
			Criteria criteria=session.createCriteria(FlightCreditNote.class);
			criteria.add(Restrictions.eq("id",Integer.parseInt(id)));
			flightCreditNote = (FlightCreditNote) criteria.uniqueResult();	
		}catch (HibernateException e) {			
			throw e; 
		}finally {
			if(session != null && session.isOpen())
		...
		return flightCreditNote;
	}

	public FlightCreditNote getCreditNoteByRowId(String id)
			throws HibernateException {
		Session sess = null;		
		FlightCreditNote hotelCreditNote = null;			
		try{
			sess = sessionFactory.openSession();			
			Criteria criteria=sess.createCriteria(FlightCreditNote.class);
			criteria.add(Restrictions.eq("id", Integer.parseInt(id)));
			hotelCreditNote = (FlightCreditNote) criteria.uniqueResult();	
	...
	public List<FlightCreditNote> getCreditNoteListByOrderRowID(Long id) {	
		...
		List<FlightCreditNote> creditNoteList = null;
		try{
			session = sessionFactory.openSession();			
			Criteria criteria = session.createCriteria(FlightCreditNote.class);
	...
	public FlightCreditNote getCreditNoteDetailsByComapnyId(int companyid, Long id) {
		FlightCreditNote creditNoteObj   = null;
		Session session = null;
		try
		{
			session = sessionFactory.openSession();			
			...
			creditNoteObj = (FlightCreditNote) criteria.uniqueResult();			
		}
		catch(Exception e){
			logger.error("---------Exception---------"+e.getMessage());
