...
public class FlightTempAirSegmentDAOIMP implements FlightTempAirSegmentDAO {
	Logger logger = Logger.getLogger(FlightTempAirSegmentDAOIMP.class);
	...
	public TempAirSegment getTAS(String flight_index) {
		...
		try {
			...
			criteria = criteria.add(Restrictions.eq("flight_index", flight_index));
			list = criteria.list();
		} catch (Exception e) {
			logger.error("Exception", e);
		} finally {
	...
	public void updateKeys(FlightBookingKeysTemp flightBookingKeys)
		...
		try {
			...
			criteria = criteria.add(Restrictions.eq("search_key",flightBookingKeys.getSearch_key()));
