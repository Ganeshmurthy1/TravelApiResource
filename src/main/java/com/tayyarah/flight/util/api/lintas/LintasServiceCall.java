...
public class LintasServiceCall {
	static final Logger logger = Logger.getLogger(LintasServiceCall.class);
	...
	public static SearchFlightResponse callSearchService(
		...
		try {
			...
			String searchURL=lintasConfig.getUrl()+"/flight/search?adult="+flightsearch.getAdult()+"&airline="+flightsearch.getAirline()+"&app_key="+lintasConfig.getApp_key()+"&arvlDate="+ArrvlDAte+"&cabinClass="+flightsearch.getCabinClass()+"&currency="+lintasConfig.DEFAULT_CURRENCY+"&depDate="+FlightWebServiceEndPointValidator.getDateYYYYMMDD(flightsearch.getDepDate())+"&destination="+flightsearch.getDestination()+"&infant="+flightsearch.getInfant()+"&kid="+flightsearch.getKid()+"&origin="+flightsearch.getOrigin()+"&trips="+flightsearch.getTrips()+"&triptype="+flightsearch.getTripType()+"&isDomestic="+isDomestic;
			...
			throws Exception
	{
		...
		flightCustomerDetails.setPaymode(lintasConfig.getPaymode());
		flightCustomerDetails.setApp_key(lintasConfig.getApp_key());
		flightCustomerDetails.setUserid(lintasConfig.getUserid());
		flightCustomerDetails.setUsername(lintasConfig.getUsername());
		...
		String url = lintasConfig.getUrl()+"/booking/details";
		...
		flightBookingResponse= LintasFlightBookingResponseParser.parseFlightBookingResponse(flightBookingResponse,flightBookingResponseOLD,orderId,FBDAO,emaildao,transactionkey,paymode,walletAmountTranferHistory,count);
