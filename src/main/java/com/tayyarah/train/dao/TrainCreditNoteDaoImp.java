...
public class TrainCreditNoteDaoImp implements TrainCreditNoteDao ,Serializable{
	public static final Logger logger = Logger.getLogger(TrainCreditNoteDaoImp.class);	
	/**
	 * By Yogesh
	 */
	...
	@Override
	public List<TrainCreditNote> getCreditNoteByUser(int id)
			throws HibernateException {		
		List<TrainCreditNote> userlist = null;
		try{
	...
	public TrainCreditNote getCreditNoteById(String id) throws HibernateException {
		...
		try{
			...
			creditNote = (TrainCreditNote) criteria.uniqueResult();	
		}catch (HibernateException e) {			
			throw e; 
		}finally {
			if(session != null && session.isOpen())
	...
	public TrainCreditNote getCreditNoteByRowId(String id)
			throws HibernateException {
		Session sess = null;		
		TrainCreditNote trainCreditNote = null;			
		try{
			sess = sessionFactory.openSession();		
			Criteria criteria=sess.createCriteria(TrainCreditNote.class);
			criteria.add(Restrictions.eq("id", Integer.parseInt(id)));
			trainCreditNote = (TrainCreditNote) criteria.uniqueResult();	
		}catch (HibernateException e) {			
			throw e; 
		}finally {
			sess.close(); 
		}
		return trainCreditNote;
	}
	...
	public TrainCreditNote getCreditNoteDetailsByComapnyId(int companyid, Long id) {
		...
		try
		{
			...
			creditNoteObj = (TrainCreditNote) criteria.uniqueResult();			
		}
		catch(Exception e){
			logger.error("---------Exception---------"+e.getMessage());
