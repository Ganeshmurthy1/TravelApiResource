...
public class VisaCreditNoteDaoImp implements VisaCreditNoteDao ,Serializable{
	public static final Logger logger = Logger.getLogger(VisaCreditNoteDaoImp.class);	
	/**
	 * By khalid
	 */
	...
	@Override
	public List<VisaCreditNote> getCreditNoteByUser(int id)
			throws HibernateException {	
		List<VisaCreditNote> userlist = null;
		try{
	...
	public VisaCreditNote getCreditNoteByRowId(String id)
			throws HibernateException {
		Session sess = null;		
		VisaCreditNote visaCreditNote = null;			
		try{
			sess = sessionFactory.openSession();			
			Criteria criteria=sess.createCriteria(VisaCreditNote.class);
			criteria.add(Restrictions.eq("id", Integer.parseInt(id)));
			visaCreditNote = (VisaCreditNote) criteria.uniqueResult();	
		}catch (HibernateException e) {			
			throw e; 
		}finally {
			sess.close(); 
		}
		return visaCreditNote;
	}
	...
	public VisaCreditNote getCreditNoteDetailsByComapnyId(int companyid, Long id) {
		...
		try
		{
			...
			creditNoteObj = (VisaCreditNote) criteria.uniqueResult();			
		}
		catch(Exception e){
			logger.error("---------Exception---------"+e.getMessage());
